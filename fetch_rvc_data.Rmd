...
spp_list<- fk2009$taxonomic_data$COMNAME
...
dt1999<- getRvcData(1999, "DRY TORT", server = 'https://www.sefsc.noaa.gov/rvc_analysis20/')
strata_abundt1999 <- getStratumAbundance(dt1999, spp_list, merge_protected = F) #Error in `$<-.data.frame`(`*tmp*`, "protected_status", value = c("all",  : replacement has 2712 rows, data has 5763
...
cat("bind all csv's\n") #concatenate and print - character string naming the file to print to
  d_strat = data_frame()
    for (f in list.files('/Users/debbiehepner/Dropbox/github/RVC/abundance_strata/FK', pattern="*.csv", full.names=T)){
      cat(' ', f,'\n')
      d_f = read_csv(
        f, #path to a file 
        progress=F, #progress: Display a progress bar
...
cat("bind all csv's\n") #concatenate and print - character string naming the file to print to
  d_strat = data_frame()
    for (f in list.files('/Users/debbiehepner/Dropbox/github/RVC/abundance_strata/DT', pattern="*.csv", full.names=T)){
      cat(' ', f,'\n')
      d_f = read_csv(
        f, #path to a file 
        progress=F, #progress: Display a progress bar
...
write_csv(d_strat, 'abundance_strata/strata_dt_abun.csv')  
...
cat("bind all csv's\n") #concatenate and print - character string naming the file to print to
  d = data_frame()
    for (f in list.files('/Users/debbiehepner/Dropbox/github/RVC/abundance_psu/DT', pattern="*.csv", full.names=T)){
      cat(' ', f,'\n')
      d_f = read_csv(
        f, #path to a file 
        progress=F, #progress: Display a progress bar
...
write_csv(d, 'abundance_psu/psu_dt_abun.csv') 
