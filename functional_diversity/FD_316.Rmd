...
if (!file.exists("functional_diversity/func_dist_rds")){
  ...
  trait_matrix_316 = read_csv('functional_diversity/species_trait_matrix_316_spp_10_25_17.csv')
...
#dend = cutree(as.dendrogram(hclust(dend, k = 3, h=NULL)))
#'dendrogram' with 2 branches and 23 members total, at height 1
dend6 = 
  color_branches(dend, k=20) %>% #color the branches based on the clusters 
  color_labels(k=20) %>%
...
#plot 
pdf("functional_diversity/functional_dendrogram_color_k20.pdf", width = 11, height = 65) 
par(pin=c(3.5,60))
#par(mar = c(3,3,3,7))
plot(
     dend6,
...
#par(mar=c(0,0,0,0)) #number of lines of margins for bottom, left, top, right
par(pin=c(3.5,60))         #plot dimensions (width, height)
...
plot(func.dist.23, horiz=TRUE, main = "Functional Dendrogram", cex = 1, cex.main = 2, cex.lab = 1, ps = 35)
dev.off()
...
library(dendextend)
func_dend_23 = as.dendrogram(func.dist.23)
play = rotate(func_dend_23, 1:23)
play2 = color_branches(func_dend_23, k=3)
