...
if (!file.exists(RVCdata_FK_rds)){
  RVCdata_FK <- getRvcData(1999:2016, "FLA KEYS", server = 'https://www.sefsc.noaa.gov/rvc_analysis20/')
  write_rds(RVCdata_FK, RVCdata_FK_rds)
}
RVCdata_FK = read_rds(RVCdata_FK_rds)
#write_csv(RVCdata_FK, 'big_csv/RVCdata_FK.csv')

if (!file.exists(RVCdata_DT_rds)){
  RVCdata_DT <- getRvcData(1999:2016, "DRY TORT", server = 'https://www.sefsc.noaa.gov/rvc_analysis20/')

  write_rds(RVCdata_DT, RVCdata_DT_rds)
}
RVCdata_FK = read_rds(RVCdata_DT_rds)
#write_csv(RVCdata_DT, 'big_csv/RVCdata_DT.csv')
...
#abundance: Estimated abundance per sampling domain

RVCdata_FKbu<-RVCdata_FK  
...
strat90<-strat94[which(strat94$YEAR<1999),]

summary(strat90)

RVCdata_FK$sample_data<-rvc94_16

RVCdata_FK$stratum_data<-rbind(strat90,RVCdata_FK$stratum_data)


summary(RVCdata_FK$stratum_data)



#need csv file from luke 

## NEED TO RERUN ALL DT data! - MH 5/17/17 
## DT data prior to 1999? 

```

#### Read in abundance and diversity csv files for trophic groups 
```{r csv files}
# on Mac in Terminal shell: ln -s ~/Dropbox/big_csv big_csv
...
tax2<-RVCdata_FK$taxonomic_data
tax2$Trophic_Level<-as.factor(tax2$Trophic_Level)
RVCdata_FK$taxonomic_data<-tax2
RVCdata_FK$taxonomic_data

#
lookup1

lookup1=subset(rvc_4tax$taxonomic_data,Group.Name1 %in% c("Requiem Sharks"),select =c("RVC.Code.x","Group.Name1"))
