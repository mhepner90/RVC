...
## Should I remove the species that are not identified to species level. Is this skewing my diversity indices? What percent of the abundance data is from species identified to the genus? 

#grep("SPE.$", domain_fk_abun$SPECIES_CD, value = T)

totalabun = sum(domain_fk_abun$abundance) #23,434,984,377 counts over 15 years in FK

new<-subset("SPE.$", domain_fk_abun$SPECIES_CD, value = T)
head(new)

(grep("SPE.$", domain_fk_abun$SPECIES_CD))
head(domain_fk_abun)
grep("SPE.$", domain_fk_abun$SPECIES_CD, value = T)

if(grep("SPE.$", domain_fk_abun$SPECIES_CD, value = T)){
    sppabun = sum(domain_fk_abun$abundance)
    }

percent_spp = (sppabun/totalabun)*100 
...
write_csv(fk_domain_abun_diversity, 'big_csv/abundance_domain/domain_fk_abun_diversity.csv')
...
fk_strata_abun_diversity = dt_domain_abun_diversity %>%
  select(YEAR, STRATA, protected_status, dom_richness, dom_simpson, dom_shannon) 
write_csv(fk_strata_abun_diversity, 'abundance_strata/strata_fk_abun_diversity.csv')
