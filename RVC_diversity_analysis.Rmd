...
partial.deviance.list = for(j in names(data[,8:14])) { 
    ...
    fullmod1=gam(formula(paste("asin(sqrt(",j,"))~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')+s(DEPTH,bs='ts')")), data=data)
    nullmod1 = gam(formula(paste("asin(sqrt(",j,"))~1")),data=data)
    
   } else { 
      fullmod2 = gam(formula(paste("log(",j,")~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES, by=YEAR, bs='ts')+s(DEPTH, bs='ts')")), data=data)
      nullmod2 = gam(formula(paste("log(",j,")~1")), data=data)
   }
