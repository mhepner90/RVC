...
dt_psu_abun_diversity = dt_psu_abun %>% 
  group_by(YEAR, PRIMARY_SAMPLE_UNIT, PROT)%>% 
  nest(-YEAR) %>% #
  mutate(
    data_wide = map(data, ~ spread(data=.x, SPECIES_CD, abundance, fill =0)), 
    psu_richness = map( #species richness
      data_wide, 
      function(x) specnumber(x %>% select(-REGION, -STRAT, -m, -var))),
    psu_simpson = map( #simpson diversity as effective number of species 
      data_wide,
      function(x) 1/(1 - diversity(x %>% select(-REGION, -STRAT, -m, -var), index = 'simpson'))),
    psu_shannon = map( #shannon diversity as effective number of species 
      data_wide,
      function(x) exp(diversity(x %>% select(-REGION, -STRAT, -m, -var), index = 'shannon')))) %>% 
  unnest(psu_richness,psu_simpson, psu_shannon)
