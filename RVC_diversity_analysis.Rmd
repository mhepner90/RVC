...
# abun2007 = strata_abunfk2007 %>%
#   select(YEAR, STRAT, PROT, SPECIES_CD, abundance) %>% 
#   group_by(YEAR, STRAT, PROT) %>% #7 groups --- 4,745*11 ---- #13 groups, 4,745*5
#   nest() #now 365 species in each 
...
#1. Read in data 
strata_diversity = read.csv("diversity_without_spe/strata_fk_abun_diversity_no_spp.csv") %>%
  filter(protected_status == "0") #only strata values in unprotected
...
#  1.670   9.942  12.542  12.025  14.441  19.560 
...
1-pt(-2.104094, 102) #pt=cummulative density function of t-dist, first number=calculated t-value; second=df
#0.9810857
...
#Bartlett's K-squared = 11.393, df = 14, p-value = 0.6549

bartlett.test(strata_diversity$simpson~strata_diversity$STRAT) #between simpson and strata
#Bartlett's K-squared = 12.154, df = 6, p-value = 0.05862
...
strata_diversity$simpson_boxcox = strata_diversity$simpson^1.4 #simpson_boxcox is now transformed using the boxcox method 
strata_diversity$simpson_boxcox = strata_diversity$simpson-1
strata_diversity$simpson_boxcox = strata_diversity$simpson/1.4
...
#strata_diversity$YEAR1 1     1.1      1.130    0.131      0.718             --- ??WHY IS DF 1 AND NOT 14?
#Residuals              101   871.5    8.629  
...
ANOVA = aov(strata_diversity$simpson_boxcox~strata_diversity$STRAT) 
  ...
  summary(ANOVA)
#                       Df  Sum Sq  Mean Sq   F value   Pr(>F)    
#strata_diversity$STRAT  6  269.5   44.91     7.148     2.56e-06 ***
#Residuals              96  603.2    6.28  
  
  ANOVA = aov(simpson~YEAR, data = strata_diversity) 
  plot(ANOVA)
  summary(ANOVA)
            #Df Sum Sq  Mean Sq   F value  Pr(>F)
#YEAR          1    2.2   2.214   0.131    0.718
#Residuals   101 1708.1  16.912
  
  ANOVA = aov(simpson~STRAT, data = strata_diversity) 
  plot(ANOVA)
  summary(ANOVA)
#            Df Sum Sq   Mean Sq  F value   Pr(>F)    
#STRAT        6  528.1   88.02    7.148     2.56e-06 ***
#Residuals   96 1182.2   12.31
