...
#3. Run ANOVA to get residuals so we can use them to test assumptions of normality and equal variances
...
#8. Test transformed data for normality, skew, and equal variance
...
psu_bio_sum %>% filter(YEAR!=2016) %$% shapiro.test(biomass_sum_bc) #shapiro.test can't handle >5000 values
...
plot(biomass_aov) #first two plots show residuals versus fitted values and qqplot of the residuals - USE THESE for checking assumptions
drop1(biomass_aov,~.,test="F") #displays Type III ANOVA table (default of most stats packages...use this one)
psu_bio_sum$resids <- residuals(biomass_aov)
psu_bio_sum$pred_val <- predict(biomass_aov)

psu_bio_sum %>% filter(YEAR!=2016) %$% shapiro.test(resids) #shapiro.test can't handle >5000 values


#10. Post-hoc pairwise comparisons
TukeyHSD(biomass_aov)
...
Categorical factos: models parametrically to determine their mean effect sizes (Zurr et al., 2009)
