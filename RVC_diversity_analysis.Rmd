...
```{r Plot by grouped strata and year} 
...
```{r Plot by 7 strata subregion and year}
...
partial.deviance.list = for(j in names(data[,8:14])) { 
  
    #Build full model and null model based on (transformed) response variable
    if(j=="richness") {
      fullmod = gam(formula(paste(j,"~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES,by=YEAR, bs='ts')+s(DEPTH, bs='ts')")), family=poisson, data=data)
      nullmod = gam(formula(paste(j,"~1")), family=poisson, data=data)
      
      } else if(j=="evenness") {
    eve_fullmod=gam(formula(paste("asin(sqrt(",j,"))~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')+s(DEPTH,bs='ts')")), data=data)
    eve_nullmod = gam(formula(paste("asin(sqrt(",j,"))~1")),data=data)
    
      } else if (j=="simpson"){ 
      simp_fullmod = gam(formula(paste("log(",j,")~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES, by=YEAR, bs='ts')+s(DEPTH, bs='ts')")), data=data)
      simp_nullmod = gam(formula(paste("log(",j,")~1")), data=data)
      
      } else if (j=="shannon") { 
      shan_fullmod = gam(formula(paste("log(",j,")~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES, by=YEAR, bs='ts')+s(DEPTH, bs='ts')")), data=data)
      shan_nullmod = gam(formula(paste("log(",j,")~1")), data=data)
     
       }else{ 
      func_fullmod = gam(formula(paste("log(",j,")~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES, by=YEAR, bs='ts')+s(DEPTH, bs='ts')")), data=data)
      func_nullmod = gam(formula(paste("log(",j,")~1")), data=data)
   }
...
#Calculate partial deviances by sequentially removing suites of predictors from the full model for all possible permutations (24). 
  ...
  #sp - a vector of smoothing parameters
  ...
  #time deviances 
   ...
   time.dev[1]= #(deviance(-time)-deviance(full))/deviance(null)
    ...
    space.dev[1] = #(deviance(-time-space)-deviance(-time))/deviance(null) 
      (deviance(update(fullmod,.~.-YEAR-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp[1]))-
      deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp))) /
      deviance(nullmod)
    
    mngt.dev[1]= #(deviance(-time-space-mngt)-deviance(-time)/deviance(null)
      (deviance(update(fullmod,.~.-YEAR-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT, sp=fullmod$sp[1]))-
      deviance(update(fullmod,.~.-YEAR, sp=fullmod$sp[1])))/
      deviance(nullmod) ###error: Supplied smoothing parameter vector is too short - ignored.
    
    env.dev[1]= #(deviance(null)-deviance(-time,-space,-mngt))/deviance(null) 
      (deviance(nullmod)-
      deviance(update(fullmod,.~.-YEAR-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT,sp=fullmod$sp[1])))/ deviance(nullmod)
    ...
    space.dev[2]= #(deviance(-time-space)-deviance(-time))/deviance(null)
      (deviance(update(fullmod,.~.-YEAR-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),
                       sp=fullmod$sp))-
         deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp)))/deviance(nullmod)
    #Supplied smoothing parameter vector is too short - ignored.
    
    env.dev[2]= #(deviance(-time-space-env)-deviance(-time))/deviance(null)
      (deviance(update(fullmod,.~.-YEAR-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts'),sp=fullmod$sp[1:2]))-
         deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp[1:2])))/deviance(nullmod)
    #Error in L[, fix.ind, drop = FALSE] : (subscript) logical subscript too long
    
    mngt.dev[2]= #(deviance(null)-deviance(-time-space-env))/deviance(null)
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-YEAR-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp[1:2])))/deviance(nullmod)
    #Error in L[, fix.ind, drop = FALSE] : (subscript) logical subscript too long
    ...
    mngt.dev[3]=
      (deviance(update(fullmod,.~.-YEAR-PROT, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-YEAR, sp=fullmod$sp)))/deviance(nullmod)
    #Warning message: In gam.setup(formula = list(pf = log(abundance_sum) ~ STRAT + PROT,  : Supplied smoothing parameter vector is too short - ignored.

    env.dev[3]=
      (deviance(update(fullmod,.~.-YEAR-PROT-STRAT-s(DEPTH,bs='ts'),sp=fullmod$sp[2]))-
         deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp[2])))/deviance(nullmod)
    #Warning message: In gam.setup(formula = list(pf = log(abundance_sum) ~ STRAT + PROT,  : Supplied smoothing parameter vector is too short - ignored.
    
    space.dev[3]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-YEAR-PROT-STRAT-s(DEPTH,bs='ts'),sp=fullmod$sp[2])))/deviance(nullmod)
    ...
    space.dev[4]= #(deviance(-time-mngt-space))
      (deviance(update(fullmod,.~.-YEAR-PROT-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp[1]))-
         deviance(update(fullmod,.~.-YEAR, sp=fullmod$sp[1])))/deviance(nullmod)
    #Warning message: In gam.setup(formula = list(pf = log(abundance_sum) ~ STRAT + PROT,  : Supplied smoothing parameter vector is too short - ignored.
    
    env.dev[4]= #deviance(null)-deviance(-time-mngt-space)/deviance(null)
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-YEAR-PROT-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),
                         sp=fullmod$sp[1])))/deviance(nullmod)
    ...
    env.dev[5]=
      (deviance(update(fullmod,.~.-YEAR-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp[2]))-
         deviance(update(fullmod,.~.-YEAR, sp=fullmod$sp[2])))/deviance(nullmod)
    #Warning message: In gam.setup(formula = list(pf = log(abundance_sum) ~ STRAT + PROT,  : Supplied smoothing parameter vector is too short - ignored.
    
    space.dev[5]=
      (deviance(update(fullmod,.~.-YEAR-STRAT-s(DEPTH,bs='ts')-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp[1:2]))-
         deviance(update(fullmod,.~.-YEAR, sp=fullmod$sp[1:2])))/deviance(nullmod)
  
    mngt.dev[5]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-YEAR-PROT-STRAT-s(DEPTH,bs='ts')-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp[1:2])))/deviance(nullmod)
    ...
    env.dev[6]=
      (deviance(update(fullmod,.~.-YEAR-STRAT-s(DEPTH,bs='ts'),sp=fullmod$sp))-
         deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp)))/deviance(nullmod)
    
    mngt.dev[6]=
      (deviance(update(fullmod,.~.-YEAR-STRAT-s(DEPTH,bs='ts')-PROT, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp)))/deviance(nullmod)
  
    space.dev[6]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-YEAR-STRAT-s(DEPTH,bs='ts')-PROT, sp=fullmod$sp)))/deviance(nullmod)

############## SPACE
    
    ###Sequence 7: space->time->environment->management 
    space.dev[7]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp))-
         deviance(fullmod))/deviance(nullmod)
    
    time.dev[7]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp)))/deviance(nullmod)
    
    mngt.dev[7]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-PROT, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp)))/deviance(nullmod)
    
    env.dev[7]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-PROT,sp=fullmod$sp)))/deviance(nullmod)

    ###Sequence 8: space->time->management->environment
    
    space.dev[8]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),
                       sp=fullmod$sp))-
         deviance(fullmod))/deviance(nullmod)
    ...
    env.dev[8]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp)))/deviance(nullmod)
    
    mngt.dev[8]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp)))/deviance(nullmod)    
    ...
    mngt.dev[9]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts')-PROT, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp)))/ deviance(nullmod)
    
     time.dev[9]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts')-PROT)))/ deviance(nullmod)
    ...
    ###Sequence 10: space->environment->time->mangement 
     ...
     env.dev[10]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp)))/ deviance(nullmod)
    
     time.dev[10]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts')-YEAR, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp)))/ deviance(nullmod)
    
     mngt.dev[10]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts')-YEAR)))/ deviance(nullmod)
    ...
    space.dev[11]=
     ...
     mngt.dev[11]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT,
                       sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),
                         sp=fullmod$sp)))/deviance(nullmod)
    
     time.dev[11]=
      (deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT-YEAR,
                       sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),
                         sp=fullmod$sp)))/deviance(nullmod)
     ...
     env.dev[11]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT-YEAR)))/deviance(nullmod)
    ...
    env.dev[12]=
     ...
     time.dev[12]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT-STRAT-s(DEPTH,bs='ts'))))/deviance(nullmod)
...
######## ENVIRONMENT
    ...
    time.dev[15]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-STRAT-s(DEPTH,bs='ts')-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT)))/deviance(nullmod)
    ...
    time.dev[17]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-STRAT-s(DEPTH,bs='ts')-PROT-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'))))/deviance(nullmod)
...
##### MANAGEMENT 
    ...
    time.dev[20]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-PROT-STRAT-s(DEPTH,bs='ts')-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'))))/deviance(nullmod)
    ...
    space.dev[21]=
      (deviance(update(fullmod,.~.-PROT-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')))-
         deviance(update(fullmod,.~.-STRAT-s(DEPTH,bs='ts'),
                         sp=fullmod$sp)))/deviance(nullmod)
    
    time.dev[21]=
      (deviance(update(fullmod,.~.-PROT-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR,
                       sp=fullmod$sp))-
         deviance(update(fullmod,.~.-STRAT-s(DEPTH,bs='ts'),
                         sp=fullmod$sp)))/deviance(nullmod)
    ...
    env.dev[21]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-PROT-s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR)))/deviance(nullmod)
...
```{r Plot by 7 strata in sections by year}
...
all_matrices_data = read_rds("")

#Melt dataframe so diversity indices are in a single column
  strata = reshape2::melt(all_matrices_data, id.vars = c("YEAR","STRAT"), measure.vars=c("abundance_sum", "biomass_sum", "evenness","richness","simpson","shannon","func.div")) # 31024 x 5
  
  #Summarize means and SEs by year, subregion, and variable
  strata=plyr::ddply(strata, c("YEAR", "STRAT", "variable"), summarize, value.mean=mean(value), value.se=plotrix::std.error(value))

#Rename levels for figure plotting
  levels(strata$variable)=c("A) Abundance", "B) Biomass", "C) Evenness", "D) Richness", "E) Simpson", "F) Shannon", "G) Functional")

#Rename regions for figure plotting
strata$STRAT=factor(strata$STRAT)
levels(strata$STRAT)=c("INPR", "MCPR", "OFPR", "FSLR","FMLR", "FDLR", "HRRF")
  
#Plot line graph
#INPR
INPR = strata %>% filter(STRAT=="INPR") %>% ggplot(.,aes(x=YEAR, y=value.mean))+
  geom_point(size=3)+
  geom_errorbar(aes(ymax=value.mean+value.se,ymin=value.mean-value.se),width=0.1)+
   geom_line(lwd=1)+
  facet_wrap(~variable,scales="free",ncol=2)+
  labs(title= "", x="Year", y="", colour = "Subregion")+
  scale_colour_manual(labels=c("INPR"), values= c("red"))+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
    theme(
      strip.text=element_text(hjust = 0),
      strip.background = element_blank(),
...
```{r plot abundance by strata indv}
...
ab_INPR = stats_all_matrices_data %>% filter(STRAT=="INPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "INPR", x="Year", y="Mean abundance")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
ab_MCPR = stats_all_matrices_data %>% filter(STRAT=="MCPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "MCPR", x="Year", y="Mean abundance")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
ab_OFPR = stats_all_matrices_data %>% filter(STRAT=="OFPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "OFPR", x="Year", y="Mean abundance")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
ab_FDLR = stats_all_matrices_data %>% filter(STRAT=="FDLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "FDLR", x="Year", y="Mean abundance")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
ab_FSLR = stats_all_matrices_data %>% filter(STRAT=="FSLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "FSLR", x="Year", y="Mean abundance")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
ab_FMLR = stats_all_matrices_data %>% filter(STRAT=="FMLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "FMLR", x="Year", y="Mean abundance")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
ab_HRRF = stats_all_matrices_data %>% filter(STRAT=="HRRF") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=abundance_mean+abundance_se,ymin=abundance_mean-abundance_se),width=0.1)+
  labs(title= "HRRF", x="Year", y="Mean abundance")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
#all plots on one page
abundance_strata <- grid.arrange(ab_INPR, ab_MCPR, ab_OFPR, ab_FSLR, ab_FMLR, ab_FDLR, ab_HRRF, ncol=2)

ggsave(file="abundance_strata.pdf", abundance_strata, width=7, height=10, path="big_csv/plots")

```

```{r plot biomass by strata indv}
...
bio_INPR = stats_all_matrices_data %>% filter(STRAT=="INPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=biomass_mean+biomass_se,ymin=biomass_mean-biomass_se),width=0.1)+
  labs(title= "INPR", x="Year", y="Mean biomass")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
bio_MCPR = stats_all_matrices_data %>% filter(STRAT=="MCPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=biomass_mean+biomass_se,ymin=biomass_mean-biomass_se),width=0.1)+
  labs(title= "MCPR", x="Year", y="Mean biomass")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
bio_OFPR = stats_all_matrices_data %>% filter(STRAT=="OFPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=biomass_mean+biomass_se,ymin=biomass_mean-biomass_se),width=0.1)+
  labs(title= "OFPR", x="Year", y="Mean biomass")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
bio_FDLR = stats_all_matrices_data %>% filter(STRAT=="FDLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=biomass_mean+biomass_se,ymin=biomass_mean-biomass_se),width=0.1)+
  labs(title= "FDLR", x="Year", y="Mean biomass")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
bio_FSLR = stats_all_matrices_data %>% filter(STRAT=="FSLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=biomass_mean+biomass_se,ymin=biomass_mean-biomass_se),width=0.1)+
  labs(title= "FSLR", x="Year", y="Mean biomass")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
bio_FMLR = stats_all_matrices_data %>% filter(STRAT=="FMLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=biomass_mean+biomass_se,ymin=biomass_mean-biomass_se),width=0.1)+
  labs(title= "FMLR", x="Year", y="Mean biomass")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
bio_HRRF = stats_all_matrices_data %>% filter(STRAT=="HRRF") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=biomass_mean+biomass_se,ymin=biomass_mean-biomass_se),width=0.1)+
  labs(title= "HRRF", x="Year", y="Mean biomass")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
#all plots on one page
biomass_strata <- grid.arrange(bio_INPR, bio_MCPR, bio_OFPR, bio_FSLR, bio_FMLR, bio_FDLR, bio_HRRF, ncol=2)
ggsave(file="biomass_strata.pdf", biomass_strata, width=7, height=10, path="big_csv/plots")

```

```{r plot richness by strata indv}
...
rich_INPR = stats_all_matrices_data %>% filter(STRAT=="INPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=richness_mean+richness_se,ymin=richness_mean-richness_se),width=0.1)+
  labs(title= "INPR", x="Year", y="Mean ENS")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
rich_MCPR = stats_all_matrices_data %>% filter(STRAT=="MCPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=richness_mean+richness_se,ymin=richness_mean-richness_se),width=0.1)+
  labs(title= "MCPR", x="Year", y="Mean ENS")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
rich_OFPR = stats_all_matrices_data %>% filter(STRAT=="OFPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=richness_mean+richness_se,ymin=richness_mean-richness_se),width=0.1)+
  labs(title= "OFPR", x="Year", y="Mean ENS")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
rich_FDLR = stats_all_matrices_data %>% filter(STRAT=="FDLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=richness_mean+richness_se,ymin=richness_mean-richness_se),width=0.1)+
  labs(title= "FDLR", x="Year", y="Mean ENS")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
rich_FSLR = stats_all_matrices_data %>% filter(STRAT=="FSLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=richness_mean+richness_se,ymin=richness_mean-richness_se),width=0.1)+
  labs(title= "FSLR", x="Year", y="Mean ENS")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
rich_FMLR = stats_all_matrices_data %>% filter(STRAT=="FMLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=richness_mean+richness_se,ymin=richness_mean-richness_se),width=0.1)+
  labs(title= "FMLR", x="Year", y="Mean ENS")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
rich_HRRF = stats_all_matrices_data %>% filter(STRAT=="HRRF") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=richness_mean+richness_se,ymin=richness_mean-richness_se),width=0.1)+
  labs(title= "HRRF", x="Year", y="Mean ENS")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
#all plots on one page
richness_strata <- grid.arrange(rich_INPR, rich_MCPR, rich_OFPR, rich_FSLR, rich_FMLR, rich_FDLR, rich_HRRF, ncol=2)

ggsave(file="richness_strata.pdf", richness_strata, width=7, height=10, path="big_csv/plots")

```

```{r plot evenness by strata indv}
...
eve_INPR = stats_all_matrices_data %>% filter(STRAT=="INPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=evenness_mean+evenness_se,ymin=evenness_mean-evenness_se),width=0.1)+
  labs(title= "INPR", x="Year", y="Mean relative evenness")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
eve_MCPR = stats_all_matrices_data %>% filter(STRAT=="MCPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=evenness_mean+evenness_se,ymin=evenness_mean-evenness_se),width=0.1)+
  labs(title= "MCPR", x="Year", y="Mean relative evenness")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
eve_OFPR = stats_all_matrices_data %>% filter(STRAT=="OFPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=evenness_mean+evenness_se,ymin=evenness_mean-evenness_se),width=0.1)+
  labs(title= "OFPR", x="Year", y="Mean relative evenness")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
eve_FDLR = stats_all_matrices_data %>% filter(STRAT=="FDLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=evenness_mean+evenness_se,ymin=evenness_mean-evenness_se),width=0.1)+
  labs(title= "FDLR", x="Year", y="Mean relative evenness")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
eve_FSLR = stats_all_matrices_data %>% filter(STRAT=="FSLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=evenness_mean+evenness_se,ymin=evenness_mean-evenness_se),width=0.1)+
  labs(title= "FSLR", x="Year", y="Mean relative evenness")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
eve_FMLR = stats_all_matrices_data %>% filter(STRAT=="FMLR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=evenness_mean+evenness_se,ymin=evenness_mean-evenness_se),width=0.1)+
  labs(title= "FMLR", x="Year", y="Mean relative evenness")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
eve_HRRF = stats_all_matrices_data %>% filter(STRAT=="HRRF") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=evenness_mean+evenness_se,ymin=evenness_mean-evenness_se),width=0.1)+
  labs(title= "HRRF", x="Year", y="Mean relative evenness")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
#all plots on one page
evenness_strata <- grid.arrange(eve_INPR, eve_MCPR, eve_OFPR, eve_FSLR, eve_FMLR, eve_FDLR, eve_HRRF, ncol=2)

ggsave(file="evenness_strata.pdf", evenness_strata, width=7, height=10, path="big_csv/plots")

```

```{r plot simpson by strata indv}
...
#all plots on one page
simpson_strata <- grid.arrange(simp_INPR, simp_MCPR, simp_OFPR, simp_FSLR, simp_FMLR, simp_FDLR, simp_HRRF, ncol=2)

ggsave(file="simpson_strata.pdf", simpson_strata, width=7, height=10, path="big_csv/plots")
```

```{r plot shannon by strata indv}
...
shan_INPR = stats_all_matrices_data %>% filter(STRAT=="INPR") %>%
  ...
  geom_point(size=3) +
  geom_errorbar(aes(ymax=shannon_mean+shannon_se,ymin=shannon_mean-shannon_se),width=0.1)+
  labs(title= "INPR", x="Year", y="Mean shannon")+
  scale_x_continuous(limits = c(1999, 2016), breaks = c(2000,2002, 2004, 2006, 2008, 2010, 2012, 2014, 2016), labels=c("2000","","2004","","2008","","2012","","2016"))+
  #scale_y_continuous(limits= c(200,1000))+
  theme_bw()+
  theme(
...
#all plots on one page
shannon_strata <- grid.arrange(shan_INPR, shan_MCPR, shan_OFPR, shan_FSLR, shan_FMLR, shan_FDLR, shan_HRRF, ncol=2)

ggsave(file="shannon_strata.pdf", shannon_strata, width=7, height=10, path="big_csv/plots")

```

```{r plot functional by strata indv}
# BY STRAT and YEAR
#func.div 
...
#all plots on one page
func.div_strata <- grid.arrange(func_INPR, func_MCPR, func_OFPR, func_FSLR, func_FMLR, func_FDLR, func_HRRF, ncol=2)

ggsave(file="func.div_strata.pdf", func.div_strata, width=7, height=10, path="big_csv/plots")
