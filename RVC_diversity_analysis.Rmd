...
# Drop communities with no species 
fd_trial_removed_zeros <- fd_trial[!rowSums(fd_trial) == 0, ]

# Scale relative abundances within each community
abundmat_2 <- t(apply(fd_trial_removed_zeros, 1, function(x) x / sum(x)))

# Perform matrix multiplication
Q <- c()

for(i in 1:nrow(abundmat_2)) { # you can probably be more efficient here but apply was being a jerk
  
  x <- abundmat_2[i, , drop = F]
...
if(!file.exists("psu_matrices_data_rds")){
  ...
  all_matrices = inner_join(psu_abun, psu_bio)
  all_matrices = inner_join(all_matrices, psu_div)
  all_matrices = inner_join(all_matrices, fd_evenness)
  all_matrices_data = inner_join(all_matrices, sample_data)
...
# functional
ggplot(stats_all_matrices_data,aes(x=YEAR,y=functional_mean, colour=STRAT, group=STRAT))+ #shape=STRAT
  geom_line(aes(group=STRAT),lwd=1) +
  geom_point(size=3) +
  geom_errorbar(aes(ymax=functional_mean+functional_se,ymin=functional_mean-functional_se),width=0.1)+
...
```{r Plot by year strata and subregion} 
...
if(!file.exists("matrices_strat_grouped_rds"){
  ...
  matrices_strat_grouped = write_rds("matrices_strat_grouped_rds")
}
