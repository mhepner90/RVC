...
## Plots of diversity indices by subregion and grouped strata across all years 
...
### JONS CODE DIDN'T WORK ON MY DATASET... NOT SURE WHY.... 

#Convert class("dataframe") into class("SpatialPointsDataFrame")
 for(i in seq_along(alphasp.list)) {
   coordinates(alphasp.list[[i]])=~LON_DEGREES+LAT_DEGREES 
   } 
...
##alphasp.list[[i]] is not a dataframe. but all the abun_sum values  

#Project lat long coordinates onto an ellipse
for(i in seq_along(alphasp.list)) { 
  sp::proj4string(alphasp.list[[i]])="+proj=longlat +ellps=GRS80 +datum=NAD83 +no_defs" }
#Error in (function (classes, fdef, mtable)  : unable to find an inherited method for function ‘proj4string<-’ for signature ‘"integer", "character"’
...
if(!file.exists("krigeall.list_rds")){
  ...
  #Error in checkForRemoteErrors(val) :  8 nodes produced errors; first error: missing values in object
  
  saveRDS(krigeall.list, "krigeall.list_rds") 
} 
  { else krigeall.list = read_RDS(krigeall.list)
}
...
lapply((krigeall.list),function(i) {
  do.call(grid.arrange,lapply((krigeall.list[[i]]),function(j) {
    ggplot()+
      ...
      coord_equal()+labs(x="",y="",title=paste(names(krigeall.list))+
                         theme_bw()+
