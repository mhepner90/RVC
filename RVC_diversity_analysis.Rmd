...
psu_fk_abun <- read_csv("big_csv/abundance_psu/psu_fk_abun.csv",
                        col_types = cols(
                          ...
                          var = col_double(),
                          abundance = col_double(),
                          protected_status = col_character())) #3,349,846 x 10
###shouldn't this dataframe have same number of rows as one for biomass? Ask Megan about this today

psu_fk_abun_no_spp = psu_fk_abun %>%
  filter(!stringr::str_detect(SPECIES_CD, 'SPE\\.$')) #348 species and 3,107,408 x 10
psu_fk_abun_no_spp_merged = psu_fk_abun_no_spp %>%
  filter(stringr::str_detect(protected_status, 'all')) #1,553,704 x 10 (this is half the number of rows as the original as it should be)
...
bartlett.test(psu_bio_sum_YRsubset$resids~psu_bio_sum_YRsubset$fYEAR) 
bartlett.test(psu_bio_sum_YRsubset$resids~psu_bio_sum_YRsubset$fSTRAT) 
...
bartlett.test(psu_bio_sum_YRsubset$resids_bc~psu_bio_sum_YRsubset$fYEAR) 
bartlett.test(psu_bio_sum_YRsubset$resids_bc~psu_bio_sum_YRsubset$fSTRAT) 
  ### our variances are still unequal across both YEAR and STRAT
...
# 3d. Bartlett test - p-value < 0.05 means data do not have equal variances across groups
bartlett.test(psu_abun_sum_YRsubset$resids~psu_abun_sum_YRsubset$fYEAR) 
bartlett.test(psu_abun_sum_YRsubset$resids~psu_abun_sum_YRsubset$fSTRAT) 
  ### 
...
# 4a. Try sqrt transform
psu_abun_sum_YRsubset$abundance_sum_sqrt = sqrt(psu_abun_sum_YRsubset$abundance_sum)
  ### this transformation did even worse than the boxcox
...
# 5d. Bartlett test - p-value < 0.05 means data do not have equal variances across groups
bartlett.test(psu_abun_sum_YRsubset$resids_bc~psu_abun_sum_YRsubset$fYEAR) 
bartlett.test(psu_abun_sum_YRsubset$resids_bc~psu_abun_sum_YRsubset$fSTRAT) 
  ### 
...
psu_diversity = read_csv("big_csv/diversity_without_spe/psu_diversity_no_spp_merged.csv",
                      col_types = cols(
                          ...
                          STRAT = col_character(),
                          SPECIES_CD = col_character(),
                          abun_merged = col_double())) # 1,553,704 x 8
...
psu_div = psu_diversity %>% 
  ...
  summarize(
    richness = first(richness),
    simpson = first(simpson),
    shannon = first(shannon)) # 4839 x 4
