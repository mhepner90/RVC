...
fk_strata_abun_diversity_no_spp = strata_fk_abun_no_spp %>% #124,116 * 12
  select(YEAR, STRAT, protected_status, SPECIES_CD, abundance) %>% #124,116*5
  filter(protected_status =='all') %>% #62,058 * 12#62,058 * 5
  group_by(YEAR, STRAT, protected_status) %>% #103 groups (15 years, 7 stratas)
  nest() %>%  # 103*4 
  mutate(   ##TODO: Why are there 678 species? why are the species repeated?
    data_wide = map(data, function(x) 
      full_join(x, trait_matrix %>% select(SPECIES_CD), by='SPECIES_CD') %>%
        mutate(abundance = ifelse(is.na(abundance), 0, abundance)) %>%
        spread(SPECIES_CD, abundance, fill =0)),
...
1-pt(calculated_t_value, df) #pt=cummulative density function of t-dist 

#8. Check for homoscedasticity: can use var.test, fligner.test, or bartlett.test
bartlett.test(strata_diversity$simpson~strata_diversity$YEAR)
...
stat_diversity = stat_diversity %>% 
  simpson+0.01 %>%
  b = boxcox(simpson~YEAR) %>%
  lamda = b$x %>% #lambda values  
  lik = b$y %>% #y is values of liklihood %>%
  bc = cbind(lamda,lik) %>%
  bc[order(-lik),] %>% #use the lamda with the highest likelihood to transform your data #lambda of 2 is best transformation
  mutate(
    simpson_boxcox = ((simpson^2)-1)/2) #simpson_boxcox is now transformed using the boxcox method 

#10. Check for normality of transformed data 
ggplot(data = stat) +
  geom_boxplot(mapping = aes(x = STRAT, y = simpson_boxcox)) 

qqnorm(stat_diversity$simpson_boxcox) 
qqline(stat_diversity$simpson_boxcox, lty=2)
...
stat_diversity = stat_diversity %>%
  ...
  correlation1 = cor.test(simpson, shannon, method = "spearman") %>%
  correlation2 = cor.test(simpson, functional, method = "spearman") %>% 
  correlation3 = cor.test(shannon, functional, method = "spearman") %>%  
  #Examine correlations between matrices using Mantel's test
  mantel.rtest(simpson, shannon) %>% #R^2
