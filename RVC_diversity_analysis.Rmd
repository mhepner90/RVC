...
subregion_dif = matrices_strat_grouped_with_subregion %>%
  select(SUBREGION_DOMAIN,YEAR,STRAT_GROUPED,abundance_mean, biomass_mean,evenness_mean, richness_mean,simpson_mean,shannon_mean,func.div_mean) %>%
  group_by(SUBREGION_DOMAIN) %>%
  summarize(
    SR_abun_mean = mean(abundance_mean),
    SR_bio_mean = mean(biomass_mean),
    SR_even_mean = mean(evenness_mean),
    SR_rich_mean = mean(richness_mean),
    SR_simp_mean = mean(simpson_mean),
    SR_shan_mean = mean(shannon_mean),
    SR_fd_mean = mean(func.div_mean))
