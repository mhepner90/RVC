...
#clusterExport(cl,varlist=c("data")) 
  #cl - a cluster object - socket cluster with 8 nodes on host ‘localhost’
  #varlist - character vector of names of objects to export

#partial.deviance.list=parLapply(cl, data, function(i){ #data needs to be a vector (atomic or list)
#do.call(rbind, lapply(list("abundance_sum","biomass_sum","richness","evenness", "simpson","shannon", "functional"),function(j){

#if only works for single element, ifelse is for vectors 
#partial.deviance.list = lapply(data[,8:14], function(j){

partial.deviance.list = for(j in names(data[,8:14])) { 
    column_data = as.data.frame(data[[j]])
    #Build full model and null model based on (transformed) response variable
    if(j=="richness") {
      fullmod = gam(formula(paste(column_data,"~YEAR+STRAT+PROT+s(LONG_DEGREES,LAT_DEGREES,by=YEAR, bs='ts')+s(DEPTH, by='ts')")), family=poisson, data=data)
      nullmod = gam(formula(paste(column_data,"~1")), family=poisson, data=data)
      
      } else if(j=="evenness") {
    fullmod=gam(formula(paste("asin(sqrt(",column_data,"))~YEAR+STRAT+PROT+s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')+s(DEPTH,bs='ts')")), data=data)
    nullmod = gam(formula(paste("asin(sqrt(",column_data,"))~1")),data=data)
    
   } else { 
      fullmod = gam(formula(paste("log(",column_data,")~YEAR+STRAT+PROT+s(LONG_DEGREES,LAT_DEGREES, by=YEAR, bs='ts')+s(DEPTH, by='ts')")), data=data)
      nullmod = gam(formula(paste("log(",column_data,")~1")), data=data)
   }
}
  ...
  #time deviances 
   ...
   time.dev[1]= #(deviance(-time)-deviance(full))/deviance(null)
    ...
    mngt.dev[1]= #(deviance(-time-space-mngt)-deviance(-time)/deviance(null)
      (deviance(update(fullmod,.~.-YEAR-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT, sp=fullmod$sp[6:9]))-
      deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp)))/
      deviance(nullmod)
    
    env.dev[1]= #(deviance(null)-deviance(-time,-space,-mngt))/deviance(null) 
      (deviance(nullmod)-
      deviance(update(fullmod,.~.-YEAR-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-PROT,sp=fullmod$sp[6:9])))/ deviance(nullmod)
    ...
    env.dev[2]= #(deviance(-time-space-env)-deviance(-time))/deviance(null)
      (deviance(update(fullmod,.~.-YEAR-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts'),sp=fullmod$sp[6:9]))-
         deviance(update(fullmod,.~.-YEAR,sp=fullmod$sp)))/deviance(nullmod)
    
    mngt.dev[2]= #(deviance(null)-deviance(-time-space-env))/deviance(null)
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-YEAR-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp[6:9])))/deviance(nullmod)
    ...
    space.dev[4]= #(deviance(-time-mngt-space))
      (deviance(update(fullmod,.~.-YEAR-PROT-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp[1:5]))-
         deviance(update(fullmod,.~.-YEAR, sp=fullmod$sp)))/deviance(nullmod)
    ...
    space.dev[5]=
      (deviance(update(fullmod,.~.-YEAR-STRAT-s(DEPTH,bs='ts')-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp))-
         deviance(update(fullmod,.~.-YEAR, sp=fullmod$sp)))/deviance(nullmod)
  
    mngt.dev[5]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-YEAR-PROT-STRAT-s(DEPTH,bs='ts')-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp)))/deviance(nullmod)
...
############## SPACE
    ...
    space.dev[7]=
      (deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp))-
         deviance(fullmod))/deviance(nullmod)
    
    time.dev[7]=
      (deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp)))/deviance(nullmod)
    
    mngt.dev[7]=
      (deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-PROT, sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp)))/deviance(nullmod)
    
    env.dev[7]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-PROT,sp=fullmod$sp)))/deviance(nullmod)
    ...
    env.dev[8]=
      (deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp))-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp[6:9])))/deviance(nullmod)
    
    mngt.dev[8]=
      (deviance(nullmod)-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-YEAR-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp)))/deviance(nullmod)    
    ...
    mngt.dev[9]=
      (deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts')-PROT, sp=fullmod$sp[6:9]))-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp[6:9])))/ deviance(nullmod)
    ...
    ###Sequence 10: space->environment->time->mangement 
     ...
     env.dev[10]=
      (deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts'), sp=fullmod$sp[6:9]))-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'),sp=fullmod$sp[6:9])))/ deviance(nullmod)
    
     time.dev[10]=
      (deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')-STRAT-s(DEPTH,bs='ts')-YEAR, sp=fullmod$sp[6:9]))-
         deviance(update(fullmod,.~.-s(LONG_DEGREES,LAT_DEGREES,by=YEAR,bs='ts'), sp=fullmod$sp[6:9])))/ deviance(nullmod)
