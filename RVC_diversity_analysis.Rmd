...
partial.deviance.list = for(j in names(data[,8:14])) { 
    ...
    fullmod=gam(formula(paste("asin(sqrt(",j,"))~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES,by=YEAR,bs='ts')+s(DEPTH,bs='ts')")), data=data)
    nullmod = gam(formula(paste("asin(sqrt(",j,"))~1")),data=data)
    
   } else { 
      fullmod = gam(formula(paste("log(",j,")~YEAR+STRAT+PROT+s(LON_DEGREES,LAT_DEGREES, by=YEAR, bs='ts')+s(DEPTH, bs='ts')")), data=data)
