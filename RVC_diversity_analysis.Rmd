...
plot(tree, label.offset=0.2, edge.width = 2, type = "cladogram")  #the label.offset argument moves the species names a bit to the right (try different values!)
...
fk_domain_abun_diversity_no_spp = domain_fk_abun_no_spp %>% #14,634 * 10
  ...
  nest(-YEAR) %>%  
  mutate(
    data_wide = purrr::map(data, ~ spread(data=.x, SPECIES_CD, abundance, fill =0)), 
    #data_wide 1*339 species 
     #length(unique(fk_domain_abun_diversity_no_spp$data_wide[1])) #1*339 
     #length(unique(fk_domain_abun_diversity_no_spp$data_wide[40])) #1*229 
     #length(unique(fk_domain_abun_diversity_no_spp$data_wide[45])) #1*234  
     #fk_domain_abun_diversity_no_spp$richness[[40]] #1*214
     #fk_domain_abun_diversity_no_spp$richness[[1]] #1*177 
    
    ### TODO: why dont length unique and specnumber values match? How to get so all species are listed in all years? 
    
    richness = purrr::map( #species richness 
      data_wide, #why are there 116 rows??? 
      function(x) specnumber(x)),
    simpson = map( #simpson diversity as effective number of species 
      data_wide,
      function(x) 1/(1 - diversity(x, index = 'simpson'))),
    shannon = purrr::map( #shannon diversity as effective number of species 
      data_wide,
      function(x) exp(diversity(x, index = 'shannon'))),
    func_div = purrr::map( #shannon diversity as effective number of species 
      data_wide, 
      function(x) dbFD((as.matrix(func.dist)), x))) %>% #x = trait matrix where rows are species and columns are traits; a = abundance matrix where rows are sites and species are columns 
  unnest(richness,shannon,simpson, func_div)

fk_domain_abun_diversity_no_spp = fk_domain_abun_diversity_no_spp %>%
  select(YEAR, protected_status,richness,shannon,simpson, func_div)
