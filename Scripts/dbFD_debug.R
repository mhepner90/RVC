...
`dbFD` <-
  function(x, a , w, w.abun = TRUE, stand.x = TRUE, 
           ord = c("podani", "metric"), asym.bin = NULL, corr = c("sqrt", "cailliez", "lingoes", "none"),  
           calc.FRic = TRUE, m = "max", stand.FRic = TRUE, scale.RaoQ = FALSE, calc.FGR = FALSE, 
           clust.type = "ward", km.inf.gr = 2, km.sup.gr = nrow(x) - 1, km.iter = 100, 
           km.crit = c("calinski", "ssi"),  calc.CWM = TRUE, CWM.type = c("dom", "all"), 
           calc.FDiv = TRUE, dist.bin = 2, print.pco = FALSE, messages = TRUE){
    
    # DEBUG: keep track of which row when used in tibble %>% mutate(x = map2(x, a, dbFD))
    i_dbg <<- i_dbg + 1
    cat(sprintf('%03d\n', i_dbg))
      ...
      {	
        ...
        if (x.pco$nf > 1)
        {
          ...
          if (stand.FRic)
          {
            ...
            FRic.all <- hull.all$vol # Error in hull.all$vol : $ operator is invalid for atomic vectors
          }
      ...
      {	
        ...
        else
        {
          if (dim(tr.FRic)[2] > 1 & nb.sp[i] >= 3)
          {
            ...
            if (nb.sp[i] >= thresh)
            {
              # dim(tr.FRic): 177 176? species by row, columns one shy of species? dimnames(tr.FRic)
              convhull <- convhulln(tr.FRic, options='FA') # does not return list with $vol, prob b/c issues seen with: convhulln(tr.FRic)...
