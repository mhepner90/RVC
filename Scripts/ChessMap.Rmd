...
Taxonomic distinctness (taxonomic tree)                                                 
Functional dendrogram                                                                   
Phylogenetic tree                                                                       
Calculating local diversity                                                             
Characterizing bivariate relationships (SPLOMs, Mantel tests)                           
Mapping local diversity                                                                 
Fitting GAMs and examining spatial signal in residuals                                  
Fitting GAMs and partitioning deviance                                                  
...
```{r  IMPORTING AND FORMATTING THE DATA}
...
#Plot mean alpha diversity by region and month for each diversity index  (figure 3)
...
```{r CHARACTERIZING BIVARIATE RELATIONSHIPS}
...
#Now repeat but instead interpolate by month across years (figure 2)
#Perform kriging interpolation for each matrix (abundance-,biomass-, and presence/absence-weighted)
...
```{r EXAMINING SPATIAL SIGNAL IN RESIDUALS}
#Extract residuals for each metric vs richness, evenness, and species diversity after fitting GAM (figure 4)
...
partial.deviance.list=parLapply(cl,seq_along(alphadiv.list),function(i) {
  ...
  env.dev[1]= #deviance(null)- deviance(-time-space)/null 
    (deviance(nullmod)-
       deviance(update(fullmod,.~.-Month-Year-s(Long,Lat,by=Month,bs='ts'),
                       sp=fullmod$sp[6:9])))/deviance(nullmod)

  #Sequence 2: time->environment->space
  time.dev[2]=
    (deviance(update(fullmod,.~.-Month-Year,
                     sp=fullmod$sp))-
       deviance(fullmod))/deviance(nullmod)
  
  env.dev[2]=
    (deviance(update(fullmod,.~.-Month-Year-s(SA,bs='ts')-s(WT,bs='ts')-s(DO,bs='ts')-s(TDEPTH,bs='ts'),
                     sp=fullmod$sp[1:5]))-
       deviance(update(fullmod,.~.-Month-Year,
  ...
  space.dev[3]=
    (deviance(update(fullmod,.~.-s(Long,Lat,by=Month,bs='ts'),
                     sp=fullmod$sp[6:9]))-
       deviance(fullmod))/deviance(nullmod)
  
  time.dev[3]=
    (deviance(update(fullmod,.~.-s(Long,Lat,by=Month,bs='ts')-Month-Year,
                     sp=fullmod$sp[6:9]))-
       deviance(update(fullmod,.~.-s(Long,Lat,by=Month,bs='ts'),
                       sp=fullmod$sp[6:9])))/deviance(nullmod)
  env.dev[3]=
  ...
  env.dev[5]=
    (deviance(update(fullmod,.~.-s(SA,bs='ts')-s(WT,bs='ts')-s(DO,bs='ts')-s(TDEPTH,bs='ts'),
                     sp=fullmod$sp[1:5]))-
       deviance(fullmod))/deviance(nullmod)
  
  time.dev[5]=
    (deviance(update(fullmod,.~.-s(SA,bs='ts')-s(WT,bs='ts')-s(DO,bs='ts')-s(TDEPTH,bs='ts')-Month-Year,
                     sp=fullmod$sp[1:5]))-
       deviance(update(fullmod,.~.-s(SA,bs='ts')-s(WT,bs='ts')-s(DO,bs='ts')-s(TDEPTH,bs='ts'),
                       sp=fullmod$sp[1:5])))/deviance(nullmod)
