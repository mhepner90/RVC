...
spp_list<- fk2011$taxonomic_data$COMNAME #399 species 
...
cat("bind all csv's\n") #concatenate and print - character string naming the file to print to
  d_strat = data_frame()
    for (f in list.files('big_csv/abundance_strata/FK', pattern="*.csv", full.names=T)){
      cat(' ', f,'\n')
      d_f = read_csv(
        f, #path to a file 
        progress=F, #progress: Display a progress bar
...
write_csv(d_strat, 'big_csv/abundance_strata/strata_fk_abun.csv')  
